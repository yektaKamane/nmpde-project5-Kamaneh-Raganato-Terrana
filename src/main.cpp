...
int main(int argc, char *argv[])
{
  ...
  NeurodegenerationModel::DiseaseType disease_type = NeurodegenerationModel::DiseaseType::AlphaSynuclein;
  ...
  if (dim == 2)
  {
    FisherKol<2> problem(mesh_file, parameter_file,disease_type);
    problem.setup();
    problem.solve();
  }
  else if (dim == 3)
  {
    FisherKol<3> problem(mesh_file, parameter_file,disease_type);
    problem.setup();
    problem.solve();
  }
  else
